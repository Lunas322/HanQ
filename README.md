...
**언어가 달라도 같은 글을 읽는 실시간 번역 Q&A 서비스.**
한국어로 질문을 올리면 일본·영어권·중화권 사용자에게는 각자의 언어로 보인다. 반대도 마찬가지다. 사용자는 번역을 의식하지 않고 자기 모국어로만 읽고 쓴다.

현재 지원 언어는 **한국어(ko) · 일본어(ja) · 영어(en) · 중국어 간체(zh)** 네 가지이고, 언어를 늘리는 비용은 사전 파일 하나와 국가 매핑 몇 줄이다.
...
질문과 답변은 작성 즉시 백그라운드에서 DeepL API를 통해 번역되어 **지원 언어별로 한 문서에 함께 저장된다.** 조회 시점에 번역하지 않기 때문에 읽기 속도에 영향이 없다.

- 작성 언어를 자동 감지하고 나머지 지원 언어 전부로 번역
- 번역이 끝나기 전에는 "번역 중" 배지를 노출
- 원문/번역 전환 토글 제공 — 번역이 어색할 때 원문을 직접 확인할 수 있다

작성 언어 감지는 문자 종류를 세어서 판단한다. 다만 한자는 일본어와 중국어가 공유하기 때문에 글자만으로는 갈리지 않는다. 가나가 섞여 있으면 일본어로 보고, 가나 없이 한자만 있으면 접속 지역에서 추론한 언어를 따른다.
...
판별 우선순위:
1. 사용자가 직접 고른 언어 (쿠키)
2. 접속 국가 (`x-vercel-ip-country` → KR·JP·CN/TW/HK 등, 영어권은 en)
3. 기본값 한국어

언어 판별 결과는 요청 헤더로 서버 컴포넌트까지 전달되어, UI 문구 사전(`ko.ts` / `ja.ts` / `en.ts` / `zh.ts`)과 데이터 조회에 동일하게 적용된다. 지원 언어 목록은 `types/language.ts` 한 곳에서 나오고, 사전·번역 코드 매핑·국가 매핑이 모두 그 타입에 묶여 있어 언어를 빠뜨리면 타입 에러로 잡힌다.
...
├── lib/
│   ├── i18n/             ko / ja / en / zh 사전, 언어 컨텍스트
│   ├── questions.ts      질문 데이터 계층
│   ├── answers.ts        답변 데이터 계층
│   ├── translate.ts      DeepL 클라이언트
│   ├── detect-language.ts 작성 언어 감지
│   ├── country-language.ts 접속 국가 → 언어 매핑
